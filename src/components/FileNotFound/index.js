...
const FileNotFound = ({ filename, filter }) => {
  ...
  const dateFrom = filter?.date?.from && new Date(filter?.date?.from);
  const dateTo = filter?.date?.to && new Date(filter?.date?.to);
  ...
  return (
    <div className={classes.root} data-testid="file-not-found-component">
      <Grid container direction="column" justify="flex-start" alignItems="center">
        <Grid item>
          <img alt="File Not Found Icon" data-testid="file-not-found-icon" src={NotFoundIcon} />
          <p>
            No results found for{" "}
            <strong data-testid="file-not-found-desc">{filename ? `${filename}` : ""}</strong>
            <ul className={classes.list}>
              ...
              {type && (
                <li>
                  Types: <strong>{type.map((x) => x.split(".").pop()).toString()}</strong>
                </li>
              )}
              {dateFrom && dateTo && (
                <li>
          ...
          {(owner || dateFrom || dateTo || type || locations) && (
            <>
